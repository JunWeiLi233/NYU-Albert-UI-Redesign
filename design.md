...
- Layout adaptations: at 900px and above use a fixed 264px scrollable application rail and offset the verified top-level native workspace without moving it; the rail separates current-area Quick access from universal NYU resources. Below 900px use a compact sticky top header and horizontally scrollable navigation as the fast path, plus a 44px-minimum “Find a task” disclosure with separate Student services, Quick access, and NYU resources sections. Student-service cards are task-first inside the finder even though the compact fast-path tabs retain Albert's native area labels. At short compact heights the finder remains independently scrollable and neither native masthead navigation nor unfocused native skip links may overlap its cards. Family and deep-page layouts reduce columns without document-level horizontal overflow. Below 900px all content follows source order in one column. Wide tables may scroll inside their marked region, but the document itself must not overflow horizontally.
