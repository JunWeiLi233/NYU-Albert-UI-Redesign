...
- Task discovery: an on-demand “Find a task” panel repeats only the currently verified areas, page-family Quick access links, and universal NYU resource links with their descriptions; it adds no URLs or availability inference. Area cards lead with the task-oriented job (“Bills, statements, and financial aid”) and identify the original destination second (“Finances · Albert area”), while their accessible name includes both the task and native area. Below 900px the panel expands within the compact header; at 900px and above its white surface fills the workspace beside the fixed rail while its content stays bounded, following NYU's full “All NYU” navigation pattern without copying public-site destinations or branding assets.
...
- Test/screenshot expectations: sanitized fixtures cover all five observed selected workspaces plus native Other Resources delegation and universal resource discovery, realistic deep forms, read-only and unknown transaction modals, Class Search success/empty/error states, authentication, unknown surfaces, and partial failure. Unit and lifecycle tests prove that resource shortcuts require one exact submenu and one exact-label direct anchor, fail open for missing/duplicate/nested/disabled controls, update after PeopleSoft replacement, and delegate to the original anchor. Browser tests prove resource availability across every selected workspace; layout regions at 400, 768, 900, 1200, and 1440px; task-first area names and combined accessible names at every supported width; desktop finder containment beside the rail at 900, 1200, and 1440px; the compact finder at 400, 768, and 899px; all three grouped sections; keyboard open/Escape/focus return and Tab-cycle containment; rail action closure before native Other Resources overlay entry; native delegation; 400×376 short-height scrolling without an unfocused native skip-link collision; native skip-link focus and activation; document overflow; 200% zoom; keyboard focus; disabled rollback; DOM replacement; missing selectors; native control identity/form ownership/click behavior; storage/network privacy; and exact Class Search host containment.
