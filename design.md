...
- Secondary tools: present allowlisted, currently present native destinations as descriptive Quick access links rather than unexplained system labels. Home may expose Course Search and Weekly Schedule; Academics may expose Academic Planner, Degree Progress Report, What If Report, and graduation status; Records may expose Enrollment Verification, Test Scores, unofficial transcripts, and transfer credit; Finances may expose balance, statement, and financial-aid status; Personal Info may expose read-only section entry points.
- Universal resources: present Academic Calendar, University Registrar, Wellness Center, and Housing in a separate “NYU resources” group on every area only when the exact native Other Resources submenu exists once and contains exactly one matching direct anchor. Hidden submenu state is expected and does not make a verified native link unavailable. Missing, duplicate, detached, disabled, or nested lookalike links are omitted. Activation delegates to the original anchor; the extension never copies or constructs its URL. This group supplements rather than replaces the native Other Resources overlay.
- Task discovery: an on-demand “Find a task” panel repeats only the currently verified areas, every unique visible enabled allowlisted page tool already present in a verified native container, and universal NYU resource links; it adds no URLs or availability inference. Area cards lead with the task-oriented job (“Bills, statements, and financial aid”) and identify the original destination second (“Finances · Albert area”). Direct tools are grouped by their Albert area but also lead with the student job (“Review aid status and requirements”) before the native tool name (“Financial Aid Status · Finances”). Accessible names include both concepts. Duplicate, hidden, disabled, detached, or out-of-boundary matches are omitted. Below 900px the panel expands within the compact header; at 900px and above its white surface fills the workspace beside the fixed rail while its content stays bounded, following NYU's full “All NYU” navigation pattern without copying public-site destinations or branding assets.
...
- Keyboard/focus behavior: shell navigation and disablement are native buttons with visible focus rings and 44px minimum targets; native document order and controls are not intercepted. Albert's original `#skiptocontent` link remains focusable and functional, but its wrapper uses the same off-canvas-until-focused pattern as the extension skip action so it cannot obscure task cards at compact heights.
...
- Layout adaptations: at 900px and above use a fixed 264px scrollable application rail and offset the verified top-level native workspace without moving it; the rail separates current-area Quick access from universal NYU resources. Its 44px-minimum “Find a task” control opens a full-height, independently scrollable panel beside the rail, never wider than the remaining viewport, with task areas spanning two columns when space permits and Quick access beside NYU resources. Below 900px use a compact sticky top header and horizontally scrollable navigation as the fast path, plus the same finder expanding in place with separate What you can do, Quick access, and NYU resources sections. Area cards are task-first inside the finder even though the compact fast-path tabs retain Albert's native area labels. At short heights the finder remains independently scrollable and neither native masthead navigation nor unfocused native skip links may overlap its cards. Family and deep-page layouts reduce columns without document-level horizontal overflow. Below 900px all content follows source order in one column. Wide tables may scroll inside their marked region, but the document itself must not overflow horizontally.
...
| Other Resources | This observed surface is a native overlay, not a selected page document. The shell delegates to Albert's original trigger and does not reconstruct or restyle the overlay without stable DOM anchors. A universal shortcut group may delegate to the four allowlisted direct anchors inside the single exact native submenu while leaving the overlay intact. Any distinct resource document still receives the generic/deep adapter. | Use only uniquely matched original native destinations; add no tracking parameters, remote metadata, copied URLs, or synthetic links. Missing or ambiguous anchors are omitted. |
